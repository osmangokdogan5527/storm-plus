...
export function IslemModal({
  ...
  const [isConvertedAmountEdited, setIsConvertedAmountEdited] =
  ...
  useEffect(() => {
    const handleHardwareScan = (e: Event) => {
      ...
      if (
        ...
        const scannedStock = stoklar.find(
          (s) => s.barcode === code || s.code === code,
        );
        if (scannedStock) {
          const existingItemIndex = invoiceItems.findIndex(
            (item) => item.stockId === scannedStock.id,
          );
          if (existingItemIndex >= 0) {
            ...
            const price =
              modalType === "sale" || modalType === "sale_return"
                ? scannedStock.salesPrice
                : scannedStock.purchasePrice;
            const emptyRowIndex = invoiceItems.findIndex(
              (item) => !item.stockId,
            );
            const newItem = {
  ...
  // Init state when modal opens or editingTransaction changes
  useEffect(() => {
    if (!isOpen) return;
    ...
    if (editingTransaction) {
      ...
      setIsMultiCurrency(
        !!(
          ...
          editingTransaction.currency !== "TRY" &&
      ...
      if (
        ...
        setCustomConvertedAmount(
          editingTransaction.convertedAmount ||
            editingTransaction.amount * editingTransaction.exchangeRate,
        );
      } else {
        setIsConvertedAmountEdited(false);
        setCustomConvertedAmount(0);
  ...
  const handleBarcodeScan = (e: React.FormEvent) => {
    ...
    const scannedStock = stoklar.find(
      (s) => s.barcode === parsed || s.code === parsed,
    );
    ...
    try {
      if (
        ...
        const islemData: Omit<Transaction, "id"> = {
          ...
          bankAccountId:
            (account === "cash" || account === "bank" || account === "pos") &&
        ...
        const islemData: Omit<Transaction, "id"> = {
          ...
          description:
            description ||
            `${modalType === "collection" ? "Tahsilat" : "Ödeme"} Makbuzu`,
          createdAt: editingTransaction?.createdAt || new Date().toISOString(),
          currency: isMultiCurrency
            ? transactionCurrency
            : selectedCari.currency || "TRY",
      ...
      console.error(err);
      setFormError(
        "İşlem kaydedilirken bir hata oluştu. Lütfen tekrar deneyin.",
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-2 md:p-4 bg-slate-900/40 backdrop-blur-xs overflow-y-auto animate-fade-in">
        <div
          className={`bg-white rounded-xl border border-slate-200 w-full shadow-2xl overflow-hidden flex flex-col max-h-[95vh] max-h-[calc(100dvh-1rem)] md:max-h-[95vh] animate-scale-in transition-all duration-300 ${isInvoice ? "max-w-5xl" : "max-w-lg"}`}
        >
          {/* Modal Header */}
          <div
            className="p-6 border-b border-slate-100 flex justify-between items-center bg-white border-l-4"
            style={{ borderLeftColor: "var(--accent-500)" }}
          >
            <div>
              ...
              <span className="text-[10px] text-slate-450 font-bold uppercase tracking-wider mt-1 block">
                {modalType === "sale"
                  ? "Müşterinize keseceğiniz faturayı ve kalemleri girin."
                  : modalType === "purchase"
                    ? "Tedarikçinizden aldığınız faturayı ve kalemleri girin."
                    : modalType === "sale_return"
                      ? "Müşterinizden gelen iade faturası ve kalemleri girin."
                      : modalType === "purchase_return"
                        ? "Tedarikçinize gönderdiğiniz iade faturası ve kalemleri girin."
                        : modalType === "collection"
                          ? "Müşterinizden yaptığınız tahsilat bilgilerini girin."
          ...
          >
            <div className="p-5 md:p-6 overflow-y-auto flex-1 space-y-5 md:space-y-6 min-h-0">
              ...
              {isInvoice ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      >
                        ...
                        {cariler
                          .filter((cari) => cari.isActive !== false)
                          .map((cari) => (
                  ...
                  {(account === "cash" ||
                    ...
                    bankAccounts.length > 0 && (
                      <div className="animate-fade-in">
                        <label className="block text-[10px] font-bold text-teal-600 uppercase tracking-wider mb-1.5 font-mono">
                          İşlem Yapılacak Kasa/Banka *
                        </label>
                        <select
                          required
                        ...
                        >
                            ...
                            >
                              <option value="">
                                {item.stockId === "" && item.stockName
                                  ? `🤖 Bulunamadı: "${item.stockName}"`
                                  : "-- Ürün / Hizmet Seçin --"}
                              </option>
                              {stoklar.map((stok) => (
                                <option key={stok.id} value={stok.id}>
                                  {stok.name}{" "}
                                  {stok.brand ? `[${stok.brand}]` : ""}{" "}
                                  {stok.category ? `(${stok.category})` : ""}{" "}
                                  (Mevcut: {stok.quantity} {stok.unit})
                                </option>
                              ))}
                            </select>
                          </div>
                          ...
                          <div className="w-full md:w-28">
                            ...
                            <div className="flex items-center border border-slate-200 rounded-md bg-white overflow-hidden focus-within:border-teal-500 transition">
                              ...
                              />
                              <span className="px-2 text-[10px] bg-slate-50 font-bold text-slate-400 border-l border-slate-100">
                                {item.unit}
                              </span>
                            </div>
                          </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-slate-100 items-end">
                      ...
                      <div className="text-left text-slate-500 text-[10px] space-y-1 font-sans">
                        {isMultiCurrency && (
                          <div className="p-3 rounded-lg bg-teal-50 border border-teal-100 text-teal-700 font-mono space-y-1 animate-fade-in">
                            <div className="font-bold uppercase tracking-wider text-[8px] text-teal-600">
                              Dövizli İşlem Aktif
                            </div>
                            <div>
                              Seçilen Kur: 1 {transactionCurrency} ={" "}
                              {exchangeRate} {activeCariCurrency}
                            </div>
                            <div>
                              Cariye Yansıyacak Tutar:{" "}
                              <span className="font-bold text-slate-900">
                                {formatCurrency(
                                  customConvertedAmount,
                                  activeCariCurrency,
                                )}
                              </span>
                            </div>
                      ...
                      <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 shadow-xs space-y-2.5 relative overflow-hidden max-w-sm ml-auto w-full">
                        <div className="flex justify-between items-center pt-1">
                          ...
                          <span
                            className="font-extrabold font-mono text-red-700"
                            style={{ fontSize: "18px" }}
                          >
                            {formatCurrency(
                              invoiceTotals.grandTotal,
                ...
                <div className="space-y-6 animate-fade-in font-sans">
                    ...
                    {(account === "cash" ||
                      ...
                      bankAccounts.length > 0 && (
                        <div className="sm:col-span-2 animate-fade-in">
                          <label className="block text-[9px] font-bold text-teal-600 uppercase tracking-widest mb-1.5 font-mono">
                  ...
                  <div className="pt-4 border-t border-slate-100">
                    <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2 font-mono">
                      İşlem Tutarı *
                    </label>
                    <div className="relative">
                      <input
                        ...
                        value={receiptAmount || ""}
                        onChange={(e) =>
                          setReceiptAmount(parseFloat(e.target.value) || 0)
                        }
                        className={`w-full px-4 py-3.5 bg-white border rounded-xl text-xl font-bold focus:outline-hidden transition font-mono ${
                          modalType === "collection"
                            ? "text-emerald-600 border-emerald-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/10"
                            : "text-rose-600 border-rose-200 focus:border-rose-500 focus:ring-1 focus:ring-rose-500/10"
                        }`}
                      />
                ...
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 space-y-3 font-sans">
                    ...
                    <label className="flex items-center gap-2 text-xs font-semibold text-slate-700 cursor-pointer select-none">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (e.target.checked) {
                            const selectedCari = cariler.find(
                              (c) => c.id === selectedCariId,
                            );
                  ...
                  {isMultiCurrency && (
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-slate-200/60 animate-fade-in">
                      <div>
                        <label className="block text-[9px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5 font-mono">
                          İşlem Birimi
                        </label>
                        <select
                          value={transactionCurrency}
                          onChange={(e) =>
                            setTransactionCurrency(e.target.value as any)
                          }
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-1">
                          ...
                          >
                            <RefreshCw
                              size={8}
                              className={tcmbLoading ? "animate-spin" : ""}
                            />
                            Canlı Kur Çek
                          </button>
                        </div>
                      ...
                      <div>
                        <label className="block text-[9px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5 font-mono">
                          Yansıyacak Tutar ({activeCariCurrency})
                        </label>
                        <input
                          type="number"
                          step="0.01"
              ...
              <div className="w-full">
                <label className="block text-[9px] font-bold text-slate-450 uppercase tracking-widest mb-1.5 font-mono">
                  İşlem Açıklaması
                </label>
                <textarea
                  id="form-islem-desc"
