...
search_input = """<div className="relative flex-1 min-w-[200px]">
          <Search
...
content = re.sub(r'<div className="relative flex-1 min-w-\[200px\]">\s*<Search\s*className="absolute left-3\.5 top-1/2 -translate-y-1/2 text-white/30"\s*size=\{16\}\s*/>\s*<input\s*id="search-cari"\s*type="text"\s*placeholder="Cari adı, kodu, telefon veya e-posta ile ara..."\s*value=\{searchTerm\}\s*onChange=\{\(e\) => setSearchTerm\(e\.target\.value\)\}\s*className="w-full bg-\[\#111\] border border-white/10 rounded-lg pl-10 pr-4 py-2\.5 text-sm text-white focus:outline-none focus:border-\[\#4ADE80\] transition-colors"\s*/>\s*</div>', search_input, content, flags=re.DOTALL)
